...
class NetworkData:
    ...
    def __init__(self, file):
        ...
        '''
        self.required_columns_ = ['B_network_feature_1','A_follower_count','B_follower_count',
                                  'A_listed_count', 'B_listed_count', 'A_network_feature_1',
        ...
        if not self.check_data():
            raise ValueError(
                'Input data do not included required features: {}'.format(self.required_columns_))
    ...
    def check_data(self):
        ...
        '''
        return all(feature in self.data_.columns for feature in self.required_columns_)
    ...
    def get_data_to_predict(self):
        ...
        '''
        pred_columns = ['AmB_listed_count', 'AmB_follower_count', 'B_network_feature_1',
                        'A_follower_m_following_count', 'A_network_feature_1',
                        'B_listed_count', 'A_following_count', 'B_following_count',
                        ]
        return self.data_[pred_columns]
...
class PredictInfluence(Resource):
    ...
    def predict_proba(self):
        ...
        predict_prob = self.clf.predict_proba(self.data.get_data_to_predict())
    ...
    def predict(self):
        # predict
        predict = self.clf.predict(self.data.get_data_to_predict())
        # Convert probabilities to DF
